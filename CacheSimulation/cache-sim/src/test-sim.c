...
int ntotal = 52; // the total number of points
...
void test_cache () {
  ...
  test(block_size == cache->block_size,
       ...
       msg("cache->block_size impropertly initialized: expected %d", block_size),
       6,
       3);
  ...
  test(!testval,
       ...
       msg("line_count in Set is not properly initialized; expected (%d)", lines),
       8,
       1);
  ...
  test(!testval,
       "accessed and block_size in Line are properly initialized",
       "accessed and block_size in Line are not properly initialized",
       12,
       2);
}
...
void test_cat_trace () {
  ...
  test(cpu->cold+cpu->conflict == 1354,
       ...
       16,
       nearness(cpu->cold+cpu->conflict, 1354, 2));
...
void test_ls_trace () {
  ...
  test(cpu->cold+cpu->conflict == 421934,
       ...
       19,
       nearness(cpu->cold+cpu->conflict, 421934, 2));
  ...
  test(cpu->conflict == 421918,
	   ...
	   cpu->conflict, 421918),
       21,
       nearness(cpu->conflict, 421918, 2));
...
void test_wc_trace () {
  ...
  test(cpu->cold+cpu->conflict == 22104,
       ...
       22,
       nearness(cpu->cold+cpu->conflict, 22104, 2));
