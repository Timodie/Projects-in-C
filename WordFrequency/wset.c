...
int wset_add (WordSet *wset, char *word) {
  ...
  WordNode** curr= search(wset,word);
 
  if(*curr==NULL){
    WordNode* newNode =node_new(word);
    newNode->next=NULL;
    newNode->freq+=1;
     *curr = newNode;
    wset->size+=1;
...
int wset_fast_add (WordSet *wset, char *word) {
  ...
  // This function is used by the wordfreq-fast.c program.
    WordNode** curr= search(wset,word);
    //WordNode* curr= wset->head;
    
    //printf("im in fast add1\n");

    if(*curr==NULL){
    WordNode* newNode =node_new(word);
    newNode->next=wset->head;
    wset->head = newNode;
    wset->size+=1;
    newNode->freq =1;
    //printf("im in fast add2\n");
    return newNode->freq;

} 
//printf("im in fastadd0\n");
if(*curr!=NULL){

WordNode* prev;
//printf("im in fastadd1\n");
...
// printf("im in fastadd2\n");

//      prev = &((*prev)->next);
//      printf("im in fastadd3a\n");

// }
//    (*prev)->next = (*curr)->next;
//    printf("im in fastadd3\n");
//    (*curr)->next = wset->head;
//    printf("im in fastadd4\n");
//   (*curr)->freq+=1;
...
int wset_remove (WordSet *wset, char *word) {
  ...
  if(wset_size(wset)==0){// function is designed to make this function easy to write!
  return -1;
}
  WordNode** curr= search(wset,word);
  WordNode* prev = NULL;
    ...
    if(*curr!=NULL){
      ...
      // }
        ...
        //printf("right in remove\n");
        int k = (*curr)->freq;
        //printf("right in remove1\n");
        (prev) =(*curr);
        (*curr)= (*curr)->next;
        //printf("right in remove2\n");
        //prev->next = NULL;
        node_free(prev);
       // printf("right in remove3\n");
...
int wset_exists (WordSet *wset, char *word) {
  ...
  //printf("now in exist\n");
  WordNode** curr = search(wset,word);//wset->head;
