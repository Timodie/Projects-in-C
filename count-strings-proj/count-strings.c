...
void count_occurrences (int n, FILE *file, Entry *entries) {
  ...
  char max[1000];
  //char *maxi =max;
  while (fgets(max ,1000,file)!=NULL){
    
    char*found;
    //printf("found : %s",strstr(max,"friend"));
    
    int i=0;
   for(i=0; i<n;i++){
    //found = strstr(max,entries[i].string);
    found = max;
      while( (found= strstr(found,entries[i].string))!= NULL){
        entries[i].count++;
        found++;
      }
        //printf("true so count is now : %d",entries[i].count);
      }
    }
